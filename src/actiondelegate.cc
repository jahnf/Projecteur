...
namespace  {
  ...
  namespace scrollverticle {
    // ---------------------------------------------------------------------------------------------
    void paint(QPainter* p, const QStyleOptionViewItem& option, const ScrollVerticleAction* /*action*/)
    {
      const auto& fm = option.fontMetrics;
      const int xPos = (option.rect.height()-fm.height()) / 2;
      NativeKeySeqEdit::drawText(xPos, *p, option, ActionDelegate::tr("Scroll Verticle"));
    }

    // ---------------------------------------------------------------------------------------------
    QSize sizeHint(const QStyleOptionViewItem& /*opt*/, const ScrollVerticleAction* /*action*/)
    {
      return QSize(100,16);
    }
  }
  ...
  namespace changevolume {
    // ---------------------------------------------------------------------------------------------
    void paint(QPainter* p, const QStyleOptionViewItem& option, const ChangeVolumeAction* /*action*/)
    {
      const auto& fm = option.fontMetrics;
      const int xPos = (option.rect.height()-fm.height()) / 2;
      NativeKeySeqEdit::drawText(xPos, *p, option, ActionDelegate::tr("Change Volume"));
    }

    // ---------------------------------------------------------------------------------------------
    QSize sizeHint(const QStyleOptionViewItem& /*opt*/, const ChangeVolumeAction* /*action*/)
    {
      return QSize(100,16);
    }
  }
                           ...
                           const QModelIndex& index) const
{
  ...
  case Action::Type::ScrollVerticle:
    scrollverticle::paint(painter, option, static_cast<ScrollVerticleAction*>(item.action.get()));
    break;
  case Action::Type::ChangeVolume:
    changevolume::paint(painter, option, static_cast<ChangeVolumeAction*>(item.action.get()));
    break;
  }
...
QSize ActionDelegate::sizeHint(const QStyleOptionViewItem& opt, const QModelIndex& index) const
{
  ...
  case Action::Type::ScrollVerticle:
    return scrollverticle::sizeHint(opt, static_cast<ScrollVerticleAction*>(item.action.get()));
  case Action::Type::ChangeVolume:
    return changevolume::sizeHint(opt, static_cast<ChangeVolumeAction*>(item.action.get()));
...
QWidget* ActionDelegate::createEditor(QWidget* parent, const Action* action) const
{
  ...
  case Action::Type::ScrollHorizontal: // None for now...
    break;
  case Action::Type::ScrollVerticle: // None for now...
    break;
  case Action::Type::ChangeVolume: // None for now...
    break;
  }
  return nullptr;
}
                               ...
                               const QModelIndex& index) const
{
  ...
  const auto symbol = [&item]() -> unsigned int {
    ...
    case Action::Type::ScrollVerticle: return Font::Icon::power_on_off_11;
    case Action::Type::ChangeVolume: return Font::Icon::power_on_off_11;
    }
    return 0;
  }();
                                           ...
                                           const QModelIndex& index, const QPoint& globalPos)
{
  ...
  static std::vector<actionEntry> items {
    ...
    {Action::Type::ScrollVerticle, Font::Icon::power_on_off_11, tr("Scroll Verticle"), ScrollVerticleAction().isRepeated()},
    {Action::Type::ChangeVolume, Font::Icon::power_on_off_11, tr("Change Volume"), ChangeVolumeAction().isRepeated()},
  };
